  ...
  expenses: router({
    ...
    quickCreate: ownerOnlyProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const { analysis, category, store, cats, stores } = await analyzeForUser(
          ctx.user.id,
          input.text,
        );
        ...
        // Tienda forzada por el cliente gana sobre la deteccion.
        const forcedStore =
          input.storeId != null
        ...
        return {
          ...
          store: effectiveStore
            ? {
                ...
                icon: effectiveStore.icon,
                color: effectiveStore.color,
              }
            : null,
        };
      }),
