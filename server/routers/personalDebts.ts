...
const createDebtSchema = z.object({
  ...
  firstInstallmentAmount: z.number().positive().nullable().optional(),
  frequencyDays: z.number().int().positive().max(365).nullable().optional(),
  ...
  debts: router({
    ...
    quickCreate: ownerOnlyProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Un prestamo puede no tener "concepto" (ej: "deuda didi 1017"). En ese
        // caso usamos el acreedor como titulo en vez de rechazar la captura.
        const debtTitle = detection.conceptName ?? detection.creditorName;
        ...
        const debt = await createDebt(ctx.user.id, {
          ...
          firstInstallmentAmount: detection.firstInstallmentAmount,
          frequencyDays: detection.frequencyDays,
